...
def handle_mul_add_block_pattern_three(model):
    node_list = get_mul_add_block(model)
    ...
    for node_dict in node_list:
        ...
        div_mul, ok = get_all_next_node_by_output(model, add1.output[0])
        if ok == 0:
            if div_mul[0].op_type == 'Div':
                div_node_ = div_mul[0]
                mul_node_ = div_mul[1]
            else:
                div_node_ = div_mul[1]
        ...
        #################################
        ###add transpose
        ts_name_div = add1.name + '_transpose_'
        ts_output_name_div = ts_name_div + '_output_'
        add_output_shape_div = values.get_tensor_shape_by_name(model, add1.output[0])
        ts_output_shape_div = [add_output_shape_div[0], add_output_shape_div[2], add_output_shape_div[1]]
        transpose_output_div = onnx.helper.make_tensor_value_info(ts_output_name_div, onnx.TensorProto.FLOAT, ts_output_shape_div)
        
        ts_node_div = onnx.helper.make_node(
                                            'Transpose',
                                            name=ts_name_div,
                                            inputs=[add1.output[0]],
                                            outputs=[ts_output_name_div],
                                            perm=[0,2,1])

        model.graph.value_info.append(transpose_output_div)

        insert_node(model, ts_node_div, div_node_)

        div_node_.input[0] = ts_output_name_div
        mul_node_.input[0] = ts_output_name_div
        #################################

        ###########################
        ###add transpose
        mul_node_, _ = get_prev_node_by_input(model, matmul2.input[0])
        
        ts_name_mul = matmul2.name + '_transpose_'
        ts_output_name_mul = ts_name_mul + '_output_'
        add_output_shape_mul = values.get_tensor_shape_by_name(model, mul_node_.output[0])
        ts_output_shape_mul = [add_output_shape_mul[0], add_output_shape_mul[2], add_output_shape_mul[1]]
        transpose_output_mul = onnx.helper.make_tensor_value_info(ts_output_name_mul, onnx.TensorProto.FLOAT, ts_output_shape_mul)
        
        ts_node_mul = onnx.helper.make_node(
                                            'Transpose',
                                            name=ts_name_mul,
                                            inputs=[mul_node_.output[0]],
                                            outputs=[ts_output_name_mul],
                                            perm=[0,2,1])

        model.graph.value_info.append(transpose_output_mul)
        ...
        rs2_node = onnx.helper.make_node(
                                            'Reshape',
                                            name=rs2_name,
                                            inputs=[ts_output_name_mul, const_shape_name],
                                            outputs=[rs2_output_name])
        ...
        insert_node(model, ts_node_mul, rs2_node)
...
def mha_optimizer(model):
    ...
    #if pattern == 3:
    #    handle_mul_add_block_pattern_three(model)    
