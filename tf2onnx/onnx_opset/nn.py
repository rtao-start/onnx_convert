...
class Pad:
    ...
    def convert_symmetric_pads(cls, ctx, node):
        ...
        two_false = ctx.make_const(utils.make_name("two_false"), np.array([False, False], np.bool_)).output[0]
...
class CropAndResize:
    ...
    def any_version_after11(cls, opset, ctx, node, **kwargs):
        ...
        cond_const = ctx.make_const(utils.make_name("cond"), np.ones((), dtype=np.bool_))
        ctx.remove_node(node.name)
        branches = {"body": g}
...
class MatrixBandPart:
    ...
    def version_7(cls, ctx, node, **kwargs):
        ...
        const_zero_bool = g.make_const(name=node_name, np_val=np.array([[0]]).astype(np.bool_))
        ...
        cond = ctx.make_const(name=node_name, np_val=np.array(1).astype(np.bool_))
